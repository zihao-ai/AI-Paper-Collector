...
def search_from_openreview(url, name, res):
...
def crawl(cache_file=None, force=False):
    ...
    dblp_conf=pd.read_csv('conf/dblp_conf.csv').to_dict('records')
    iclr_conf = json.load(open("conf/open_conf.json", "r"))
    ...
    for conf in tqdm(iclr_conf, desc="[+] Crawling Openreview", dynamic_ncols=True):
        assert conf.get("name") and conf.get("url")
        url, name = conf["url"].strip(), conf["name"].strip()
        if name in cache_conf:
            continue
        res = search_from_openreview(url, name, res)
